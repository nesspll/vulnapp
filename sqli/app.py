...
# HERE WE IMPORT CSRF-MIDDLEWARE for CROSS SITE SCRIPTING PROTECTION
...
def init(argv):
    ...
    app = Application(
        ...
        middlewares=[
            session_middleware,
            csrf_middleware, # Here we enable into the application.
            error_middleware,
        ]
    )
    app['config'] = config
    ...
    setup_jinja(app, loader=PackageLoader('sqli', 'templates'),
                ...
                autoescape=True) #HERE we enable autoescape to prevent XSS injections
    setup_database(app)
    setup_redis(app)
    setup_routes(app)
